...
### 로그인 / 회원가입 Cold Start 안내
- 로그인과 회원가입 모두 첫 요청에서 서버 준비 시간이 걸릴 수 있음을 안내합니다.
- 회원가입도 로그인과 동일한 예상 진행률 및 요청 타임아웃 UI를 사용합니다.
- 서버가 이미 실행 중이면 즉시 처리하고, Render가 대기 상태이면 예상 대기 진행률을 표시합니다.
